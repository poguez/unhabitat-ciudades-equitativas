function graphgini(data,pais,ciudad) {
    ...
    d3.csv(data, function(error, csv) {
        ...
        yearmincit = parseDate(csv[0]['Ano']);
        var csvlen= csv.length;
        for (var i = 0; i < csvlen; i++) {
            year = parseDate(csv[i]['Ano']);
            gini = +csv[i]['GINI'];
            if(gini) {
                giniData = { year: year, gini: gini, city: ciudad };
        ...
        for (var i = 0; i < csvnacionallen; i++) {
            year = parseDate(csvnacional[i]['Ano']);
            gini = +csvnacional[i]['GINI'];
            if (year >= yearmincit) {
                if(gini) {
                    giniData = { year: year, gini: gini, city: pais };
    ...
    xAxis.tickValues(giniCityData.map(function(d){return d["year"];}));
